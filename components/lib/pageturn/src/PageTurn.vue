    ...
    <div class="page">
        ...
        <div class="pageturn">
            <div>
                ...
                <p @click="goToPage(gopage)" :style="pageStyle" >Go</p>
            </div>

            <!-- 上一页显示 -->
            <p class="last-page" :style="pageStyle" v-if="curpage!==1" @click="beforePage(curpage-1)">上一页</p>
            <p class="page-disabled" v-else >上一页</p>

            <!-- 正常显示 -->
            <template v-if="!Toolong"  >
                <p v-for="p in page" :key="p" @click="goToPage(p)" :class="{pageselect:p==curpage}" :style="pageStyle" >{{p}}</p>
            </template>
            <template v-else>
                <p v-for="p in plus"  :class="{pageselect:p==curpage}"  @click="goToPage(p)" :key="p" :style="pageStyle" >{{p}}</p>
            </template>
            <!-- 超出正常显示范围 -->
            
            <span v-if="Toolong&&(curpage!==page)&&(page-curpage>4)">...</span>
            <p v-if="Toolong" :style="pageStyle" @click="goToPage(page)" :class="{pageselect:curpage==page} ">{{page}}</p>


            <p class="next-page" :style="pageStyle" v-if="curpage!==page" @click="nextPage(curpage+1)">下一页</p>
            <p class="page-disabled" v-else >下一页</p>
            </div>
    </div>
</template>
